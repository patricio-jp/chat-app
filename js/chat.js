...
function mostrarUsuariosEnLinea() {
  ...
  usuariosEnLinea.forEach(user => {
    if (user !== usuario) {
      ...
      userItem.className = "p-2 bg-gray-100 rounded mb-1 cursor-pointer hover:bg-blue-200";
      userItem.textContent = user;

      // Resaltar al usuario con el que estamos chateando
      if (user === receptorActivo) {
        userItem.classList.add("bg-blue-300", "font-semibold");
      }
...
function mostrarMensajesCon(receptor) {
  ...
  const conversacion = historial.filter(m =>
  ...
  conversacion.forEach(mensaje => {
    ...
    msg.textContent = `${mensaje.texto} (${mensaje.hora})`;
